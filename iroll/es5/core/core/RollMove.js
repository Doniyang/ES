...
export default class RollMove {
    constructor() { }
    isIarge(first, second) {
        return first > second;
    }
    isPlenty(now, start) {
    ...
    execute(e, attrs, proxy) {
        ...
        let point = Event.isTouchEvent(e) ? e.touches[0] : e, deltaX = point.pageX - attrs.getPointX(), deltaY = point.pageY - attrs.getPointY(), pos = proxy.getPosition(), maxScroll = proxy.getMaxScroll(), timestamp = Date.now(), newX, newY, absDistX, absDistY;
        ...
        if (this.isPlenty(timestamp, attrs.getEndTime()) && (this.isIarge(10, absDistX) && this.isIarge(10, absDistY))) {
            return;
        }
