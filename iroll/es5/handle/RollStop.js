...
export default class RollStop {
    ...
    execute(e, attrs, proxy) {
        const pos = proxy.getPosition(), scope = proxy.getScope(), duration = DateKit.getTime() - attrs.getStartTime();
        let newX = Math.round(pos.x), newY = Math.round(pos.y), time = 0, easing, momentumX, momentumY;
        proxy.setState(0);
